  ...
  <div>
    <div class="crumb-title">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-likefill"></i> 我的粉丝({{query.totalNum}}人)
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-row>
            ...
            <ul class="list" v-infinite-scroll="loadUserList"  :infinite-scroll-distance="10" :infinite-scroll-delay="800" :infinite-scroll-immediate="true" style="overflow:auto">
              <el-card class="blog-card" v-for="item in userList">
                <el-row>
                  ...
                  <el-col :span="20">
                    <el-row>
                      <el-link class="info-name" :href="item.fansId ? '/blogger?id='+item.fansId : '#'" target="_blank">{{item.userName}}</el-link>
                    </el-row>
...
export default {
  ...
  setup(){
    ...
    const loadUserList = () =>{
      if (more.value){
        axios.$http.get(request.fansList+query.pageNum, null).then(function (res) {
          if (res.data){
            query.pageNum++;//页码增加
    ...
    const followAuthor = (item) => {
      axios.$http.post(request.follow, {id:item.fansId}).then(function (res) {
        item.fan = res.data;
      });
    }
